...
Quadtree::Quadtree() : Quadtree(5, 5, 0, {0.0f, 0.0f, 1600, 900}, nullptr) {}

Quadtree::Quadtree(int maxObjects, int maxLevels, 
...
void Quadtree::insertObject(std::shared_ptr<BoxColliderComponent> object)
{
    if (children[0] != nullptr)
    {
        ...
        {
            children[indexToPlaceObject] -> insertObject(object);
            return;
        }
    }
    ...
    if (objects.size() > maxObjects && level < maxLevels && children[0] == nullptr)
    {
        ...
        while (objIterator != objects.end())
        {
            ...
            if (indexToPlaceObject != thisTree)
            {
                children[indexToPlaceObject] -> insertObject(obj);
...
void Quadtree::removeObject(std::shared_ptr<BoxColliderComponent> object)
{
    ...
    if (index == thisTree || children[index] == nullptr)
    {
        ...
        {
            if (objects.at(i) -> object -> instanceId -> getInstanceId() == object -> object -> instanceId -> getInstanceId())
            {
    ...
    {
        return children[index] -> removeObject(object);
    }
}
